# File name: attempt_fix_OOM_script_backpatching_experiment.py
# OOM fix: cached_activations moved to CPU after generation; slices moved to GPU on demand in hooks.
...
def backpatching(
    ...
    if cached_activations is None:
            ...
            ).cpu()  # Shape: (n_layers, n_prompts, seq_len, d_model) — kept on CPU to avoid OOM
